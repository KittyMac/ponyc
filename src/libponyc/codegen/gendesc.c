...
#define DESC_PRIORITY 15
#define DESC_BATCH 16
#define DESC_use_main_thread 17
#define DESC_EVENT_NOTIFY 18
...
void gendesc_basetype(compile_t* c, LLVMTypeRef desc_type)
{
  ...
  params[DESC_PRIORITY] = c->priority_fn;
  params[DESC_BATCH] = c->batch_fn;
  params[DESC_use_main_thread] = c->use_main_thread_fn;
...
void gendesc_init(compile_t* c, reach_type_t* t)
{
  ...
  args[DESC_PRIORITY] = make_desc_ptr(c_t->priority_fn, c->priority_fn);
  args[DESC_BATCH] = make_desc_ptr(c_t->batch_fn, c->batch_fn);
  args[DESC_use_main_thread] = make_desc_ptr(c_t->use_main_thread_fn, c->use_main_thread_fn);
