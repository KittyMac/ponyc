  ...
  reach_method_name_t* n, reach_method_t* m)
{
  ...
  } else if(n->name == c->str__tag) {
    pony_assert(c_t->tag_fn == NULL);
    c_t->tag_fn = c_m->func;
    LLVMSetFunctionCallConv(c_m->func, LLVMCCallConv);
    LLVMSetLinkage(c_m->func, LLVMExternalLinkage);
  } else if(n->name == c->str__freed) {
    pony_assert(c_t->freed_fn == NULL);
    c_t->freed_fn = c_m->func;
    LLVMSetFunctionCallConv(c_m->func, LLVMCCallConv);
    LLVMSetLinkage(c_m->func, LLVMExternalLinkage);
  } else if(n->name == c->str__priority) {
      pony_assert(c_t->priority_fn == NULL);
      c_t->priority_fn = c_m->func;
      LLVMSetFunctionCallConv(c_m->func, LLVMCCallConv);
      LLVMSetLinkage(c_m->func, LLVMExternalLinkage);
  } else if(n->name == c->str__batch) {
      pony_assert(c_t->batch_fn == NULL);
...
static bool genfun_allocator(compile_t* c, reach_type_t* t)
{
  ...
  if((c_t->primitive != NULL) || is_pointer(t->ast) || is_unsafe_pointer(t->ast) || is_nullable_pointer(t->ast))
    return true;
