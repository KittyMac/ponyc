...
static int print_pointer_type(compile_t* c, printbuf_t* buf, ast_t* type)
{
  ...
  if(is_pointer(elem) || is_unsafe_pointer(elem) || is_nullable_pointer(elem))
...
{
  if(is_pointer(type) || is_unsafe_pointer(type) || is_nullable_pointer(type))
  {
...
static void print_types(compile_t* c, FILE* fp, printbuf_t* buf)
{
  ...
  while((t = reach_types_next(&c->reach->types, &i)) != NULL)
  {
    ...
    if(!is_pointer(t->ast) && !is_unsafe_pointer(t->ast) && !is_nullable_pointer(t->ast) && !is_machine_word(t->ast))
    {
